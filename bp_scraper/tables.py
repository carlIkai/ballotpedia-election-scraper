...
from .constants import PCT_RE, AGG_WRITEIN_PAT, WRITEIN_SUFFIX
...
def _determine_incumbent(cand_cell: Tag) -> bool:
    ...
    if has_bold and has_underline: return True
...
def _infer_party_from_name(name: str) -> Optional[str]:
    from .constants import PAREN_PARTY, EXTRA_PARTY_KEYS
...
def _infer_party_from_cells(tds: List[Tag]) -> Optional[str]:
    from .constants import EXTRA_PARTY_KEYS
    text = " ".join(re.sub(r"\s+", " ", td.get_text(" ").strip()).lower() for td in tds)
    for key, proper in EXTRA_PARTY_KEYS.items():
        if key in text: 
            return proper
    for key, proper in [
        ("democrat","Democratic"),("democratic","Democratic"),("republican","Republican"),
        ("libertarian","Libertarian"),("green","Green"),("constitution","Constitution"),
        ("progressive","Progressive"),("working families","Working Families"),
        ("american independent","American Independent"),("independent american","Independent American"),
        ("aloha ʻāina","Aloha ʻĀina"),("aloha aina","Aloha ʻĀina"),("independent","Independent"),
...
def parse_results_table(section_root: BeautifulSoup, state: str, year: int, race_label: str, source_url: str) -> List[dict]:
    rows: List[dict] = []
        ...
        for tds in row_cells:
            ...
            for td in tds:
                if "votebox-results-cell--text" in " ".join(td.get("class", [])):
                    cand_cell = td; break
            if cand_cell is None: cand_cell = tds[0]

            candidate_raw = re.sub(r"\s+"," ", cand_cell.get_text(" ").strip())
            if (not candidate_raw) or (candidate_raw.lower() in {"candidate","total","overall","source","notes"}): continue
            if AGG_WRITEIN_PAT.match(candidate_raw): continue
            ...
            if total_votes is None:
                ...
                for td in tds:
                    ...
                    if iv is not None and (best is None or iv > best): best = iv
                total_votes = best

            from .tables import _infer_party_from_cells as infer_cells  # avoid circular import
