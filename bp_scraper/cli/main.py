...
import pandas as pd
from bp_scraper.parsing.discovery import discover_state_pages, discover_house_district_pages
...
from bp_scraper.transform.unify import (
    build_position_elections, position_elections_to_rows, slugify, race_title_for_chamber
)
from bp_scraper.parsing.normalize import normalize_state_name, normalize_state_filter_arg, display_clean_name, display_clean_list
    ...
    if (args.chamber == "house") and (not races_df.empty):
        ...
        if mapped:
            races_df.loc[mask, "house_label"] = races_df.loc[mask].apply(
                lambda r: rewrite_race_label(r["state"], r["year"], r["race"], mapped), axis=1
            )
        else:
