...
from .utils import (
    normalize_state_name, norm_name
)
...
def _party_from_rows_and_cards(rows: List[dict], cards: List[dict]) -> Optional[str]:
    ...
    inferred: List[str] = []
    for row in rows:
        if row.get("party_hint"): inferred.append(row["party_hint"])
        else:
            party = _infer_party_from_name(row.get("candidate",""))
            if party: inferred.append(party)
    if not inferred and cards:
        for card in cards:
            if card.get("party"): inferred.append(card["party"])
    uniq = {x for x in inferred if x}
    if len(uniq) == 1: return next(iter(uniq))
    return None
...
def _state_from_url(url: str, year: int) -> str:
    ...
    if match_tail_fallback:
        ...
        guess = guess.replace("’","'")
        guess = _re.sub(r"^(United States House(?: of Representatives)? elections? in|United States Senate (?:special )?election in)\s+", "", guess, flags=_re.I)
...
def scrape_page(url: str, year: int, chamber: str, verbose: bool=False, primary: bool=False, delay: Optional[float]=None, retries: Optional[int]=None) -> Tuple[List[dict], List[Dict[str, object]], List[dict]]:
    ...
    if not primary:
        ...
        if not cards:
            from .tables import _infer_party_from_name
            cards = [{
                "state": state,"race": base_race_label,"year": year,"name": r["candidate"],
                "name_clean": r["candidate_clean"],"party": r.get("party_hint") or _infer_party_from_name(r["candidate"]),
                "incumbent": bool(r.get("incumbent", False)),"total_votes": r.get("total_votes"),"source_url": url,
        ...
        for r in cand_rows_all:
            key = r["candidate_clean"]; rvotes = r.get("total_votes")
        ...
        summary = {
            "state": state, "race": base_race_label, "year": year, "source_url": url,
            "is_jungle_primary": False, "primary_party": None
        }
        ...
        if verbose: print(f"[{state}] winner={summary['winner_name']} ({base_race_label})")
        ...
        for sec, label_in in sections:
            ...
            if not cand_rows:
                ...
                if adv:
                    fabricated_rows = [{
                        "state": state,"race": label,"year": year,"candidate": adv,
                        "candidate_clean": norm_name(adv),"pct": None,"total_votes": None,
                        "incumbent": bool(re.search(r"\bincumbent\b", adv, flags=re.I)),
                        "party_hint": None,"source_url": url,
                    }]
            ...
            for r in cand_rows:
                if r["candidate_clean"] not in card_names:
                    cards.append({
                        "state": state,"race": label,"year": year,"name": r["candidate"],
                        "name_clean": r["candidate_clean"],"party": r.get("party_hint") or _infer_party_from_name(r["candidate"]),
                        "incumbent": bool(r.get("incumbent", False)),"total_votes": r.get("total_votes"),"source_url": url,
                    })
            ...
            if label.endswith(" — Primary"):
                inferred_party = (cancelled_party or _party_from_rows_and_cards(cand_rows, cards) or scan_section_party_keywords(sec))
                ...
                if inferred_party:
                    ...
                    for c in cards: c["race"] = label
                    for r in cand_rows: r["race"] = label
                else:
            ...
            for c in cards:
                ck = c["name_clean"]; c["race"] = label
                c["incumbent"] = incumbency_map.get(ck, False)
                c.setdefault("total_votes", votes_map.get(ck))

            is_jungle, jungle_top = _is_jungle_label(state, label)
            summary = {
                "state": state,"race": label,"year": year,"source_url": url,
...
def _extract_canceled_primary_party(sec: BeautifulSoup) -> Optional[str]:
    ...
    from .utils import nws
    CANCELLED_PRIMARY_PARTY_RE = re.compile(r"\b([A-Za-zʻ’\-\s]+?)\s+primary\s+election\s+was\s+canceled\b", re.I)
    txt = nws(sec.get_text(" "))
    ...
    for key, val in PRIMARY_PARTY_FROM_LABEL.items():
        ...
        if base in raw: return val
    for key, val in EXTRA_PARTY_KEYS.items():
        if key in raw: return val
    for key, val in [
        ("democratic","Democratic"),("republican","Republican"),("libertarian","Libertarian"),
        ("green","Green"),("working families","Working Families"),("constitution","Constitution"),
        ("progressive","Progressive"),("independent american","Independent American"),
        ("aloha aina","Aloha ʻĀina"),("aloha ʻāina","Aloha ʻĀina"),("independent","Independent"),
    ]:
        if key in raw: return val
    return None
